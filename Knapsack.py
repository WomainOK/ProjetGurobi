...
def solve_knapsack_model(values, weights, capacity):
    ...
    dict_values = {i: (values[i], weights[i]) for i in range(num_items)}
    ...
    with gp.Env() as env:
        with gp.Model(name="knapsack", env=env) as model:
            ...
            model.setObjective(
                gp.quicksum(dict_values[i][0] * x[i] for i in range(num_items)),
                GRB.MAXIMIZE
            )

            # Define capacity constraint using the Model.addConstr() method
            model.addConstr(
                gp.quicksum(dict_values[i][1] * x[i] for i in range(num_items)) <= capacity,
